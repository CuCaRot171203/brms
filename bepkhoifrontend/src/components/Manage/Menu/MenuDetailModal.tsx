      ...
      onOk: async () => {
        try {
          const res = await axios.delete(
            `${API_BASE_URL}/api/Menu/${data.productId}`,
            {
              headers: {
          ...
          if (error.response?.status === 401) {
            ...
            message.error(
              error.response?.data?.message || "Xóa món ăn thất bại!"
            );
          }
        }
      },
          ...
          <div className="flex gap-6">
            ...
            <div className="w-7/12 flex flex-col justify-between min-h-[300px] text-sm">
              <div className="space-y-2">
                <p>
                  <strong>Mã hàng hóa:</strong> {data.productId}
                </p>
                <p>
                  <strong>Loại thực đơn:</strong> Đồ ăn
                </p>
                <p>
                  <strong>Giá vốn:</strong> {data.costPrice?.toLocaleString()}đ
                </p>
                <p>
                  <strong>Giá bán:</strong> {data.salePrice?.toLocaleString()}đ
                </p>
                <p>
                  <strong>Đơn vị:</strong> Xuất
                </p>
                <p>
